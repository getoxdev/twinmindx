...
class SummaryRepositoryImpl @Inject constructor(
    ...
    override fun observeSummary(meetingId: String): Flow<Summary?> =
        summaryDao.observeSummary(meetingId).map { it?.toDomain() }

    override suspend fun getSummary(meetingId: String): Summary? =
        summaryDao.getSummary(meetingId)?.toDomain()

    override suspend fun enqueueSummaryGeneration(meetingId: String) {
    ...
    override suspend fun retrySummaryGeneration(meetingId: String) {
        val now = System.currentTimeMillis()
        val existing = summaryDao.getSummary(meetingId)
        if (existing != null) {
